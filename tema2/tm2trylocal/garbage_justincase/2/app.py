...
def temperaturesPost():
    ...
    insertQuery = "INSERT INTO temperaturi (valoare, timestamp, id_oras) "
    insertQuery += "VALUES ('" + str(payload['valoare']) + "', '" + str(datetime.datetime.now()) + "', " + str(payload['idOras']) + ")"
    
    
    print(insertQuery)
    # try:
    #     float(payload['valoare'])
    # except:
    #     return Response(status=HTTPStatus.CONFLICT)
