...
def temperaturesGet():
    ...
    if lat:
        print(lat)
        for c in cities:
            if abs(float(c[3]) - float(lat)) > 1e-9:
                delCityList.add(c)

    if lon:
        for c in cities:
            if abs(float(c[4]) - float(lon)) > 1e-9:
                delCityList.add(c)

    res = deleteTempByCities(temperatures, delCityList)

    for i in range(1,4):
        temperatures.pop()
    print(temperatures)
    #print("remained: ", res)
    # return convertTemperaturesToJSON(res), 200
    return convertTemperaturesToJSON(temperatures), 200
