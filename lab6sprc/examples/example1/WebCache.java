...
public class WebCache {
	...
	public String requestURL(String url) {
		...
		if (cache.containsKey(url)) { // daca deja avem in cache continutul
			var entry = (WebCacheEntry)cache.get(url);
			System.out.println("Pagina " + entry.getURL() + " era deja in cache, " + entry.getFrequency() + " accesari");
			...
			while (sizeOnDisk > maxSizeOnDisk) { // cat timp am depasit mai stergem intrari
				...
				for (Enumeration en = cache.keys(); en.hasMoreElements(); ) {
					...
					if (wce == null) wce = e;
						...
						// alegem pagina cu cel mai mic numar de accesari pentru stergere (LFU)
						if (wce.getFrequency() >= e.getFrequency()) wce = e;
					}
				}
				if (wce == null) break;
				this.printWebCache();
				...
				System.out.println("Eliminare " + wce.getURL() + " din cache, a avut " + wce.getFrequency() + " accesari (LFU)");
			}
		}

		System.out.println("Pagina " + entry.getURL() + " a fost adaugata in cache, " + entry.getFrequency() + " accesari");
		return content;
	}

	public void printWebCache() {
		System.out.println("\nContinut cache:");
		for (Enumeration en = cache.keys(); en.hasMoreElements(); ) {
			String su = (String) en.nextElement();
			WebCacheEntry e = (WebCacheEntry) cache.get(su);
			System.out.println(e.getURL() + ", " + e.getFrequency() + " accesari");
		}
	}
	...
	private void purgeOldPages() {
		...
		for (Enumeration en = cache.keys(); en.hasMoreElements(); ) {
			...
			if (diff > maxTimeToKeepPage) {
				//System.out.println("Eliminare " + url + " prea vechi");
